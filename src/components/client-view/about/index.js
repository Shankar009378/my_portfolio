...
import aboutMeImage from "../../../assets/about3.jpg";
  ...
  return (
    <div className="max-w-screen-xl mt-24 mb-6 sm:mt-14 sm:mb-14 px-6 sm:px-8 lg:px-16 mx-auto" id="about">
      <div className="w-full flex">
        <AnimationWrapper className="rounded-lg w-full grid-flow-row grid grid-cols-1 sm:grid-cols-3 py-9 divide-y-2 sm:divide-y-0 sm:divide-x-2 divide-purple-main bg-ehite-500 z-10">
            ...
            >
              <div className="flex m-0 w-40 sm:w-auto">
                <div className="flex flex-col">
                  <p className="text-[50px] text-purple-main font-bold">
                    {infoItem.value}+
                  </p>
        ...
        <div className="flex flex-col justify-center items-center row-start-2 sm:row-start-1">
          <h1 className="leading-[70px] mb-4 text-3xl lg:text-4xl xl:text-5xl font-medium">
            {headingText.split(" ").map((item, index) => (
              <span
                className={`${index === 6 ? "text-purple-main" : "text-[#000]"}`}
              >
                {item}{" "}
              </span>
            ))}
      ...
      <div className="grid grid-flow-row sm:grid-flow-col grid-cols-1 sm:grid-cols-2 gap-8">
        <AnimationWrapper className="flex w-full">
          <motion.div variants={setVariants} className="h-full w-full p-4">
            <motion.div
              ...
              style={{ overflow: "hidden", borderRadius: "10px", cursor: "pointer" }}
            >
              <Image
                src={aboutMeImage}
                alt="About Me"
        ...
        <AnimationWrapper className={"flex items-center w-full p-4"}>
          <motion.div
            variants={setVariants}
            className="grid gap-4 grid-cols-3 h-full max-h-[400px] w-full"
          >
            {data?.skills && data.skills.split(",").map((skill) => (
              ...
              >
                <motion.button
                  className="whitespace-nowrap text-ellipsis overflow-hidden py-3 w-[160px] px-6 border-[2px] border-purple-main bg-[#fff] text-[#000] font-semibold rounded-lg text-xl hover:shadow-purple-main transition-all outline-none"
                  whileHover={{ scale: 1.05, boxShadow: "0px 5px 15px rgba(111, 66, 193, 0.5)" }}
                  whileTap={{ scale: 0.95 }}
                  title={skill}
                >
                  {skill}
